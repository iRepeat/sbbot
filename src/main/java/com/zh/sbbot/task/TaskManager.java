...
public class TaskManager implements InitializingBean {
    ...
    public TaskModel setTaskStatus(Long key, @Nonnull String status) {
        ...
        Assert.state(Set.of(TaskStatus.ENABLE, TaskStatus.DISABLE).contains(dbTask.getStatus()),
                "Cannot enable tasks with status: [%s, %s]".formatted(TaskStatus.INVALID, TaskStatus.DEL));

        switch (status) {
            case TaskStatus.DEL, TaskStatus.DISABLE, TaskStatus.INVALID -> {
                cancelTask(key);
