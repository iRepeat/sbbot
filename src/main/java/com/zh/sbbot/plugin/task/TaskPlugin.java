...
public class TaskPlugin {
    ...
    public void task(AnyMessageEvent event, Matcher matcher) {
        ...
        switch (commandParam) {
            ...
            case TaskStatus.ENABLE, TaskStatus.DISABLE, TaskStatus.DEL -> {
                String key = BotUtil.getParam(matcher);
                if (!StringUtils.hasText(key)) {
                    botHelper.reply(event, "参数错误");
                    return;
            ...
            default -> {
                String usage = """
                        ...
                        .formatted(TaskStatus.ENABLE, TaskStatus.DISABLE, TaskStatus.DEL);
    ...
    private String generateCron(String desc) {
        ...
        String systemTemplate = """
                你是专业工程师，你现在是职能是写cron表达式（spring schedule的格式: {秒数} {分钟} {小时} {日期} {月份} {星期} {年份(可为空)}）。\
                当用户向你表达“需求”时，你响应对应的cron（不要做任何解释）；\
                你可以参考当前时间{%s}，从而智能、尽最大努力理解用户的需求；\
